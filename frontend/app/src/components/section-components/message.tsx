...
function OtherUserMessage({ image, message }: Message)
{
    return (
...
function CurrentUserMessage({ image, message }: Message)
{
    return (
...
function Message()
{
    return (
        <div className="p-5 flex flex-col gap-4">
                {datas.map((data) => 
                    data.currentUser ?
                    <CurrentUserMessage key={data.id} message={data.message} image={data.image}/>
                    : <OtherUserMessage key={data.id} message={data.message} image={data.image}/>
                )}
        </div>
    )
}
