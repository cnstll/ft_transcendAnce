...
function CreateChannelForm(props: CreateChannelFormProps) {
  ...
  async function OnSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3000/channels/create', formData);
      ...
      <div className="grid gap-y-6">
        <form onSubmit={OnSubmit}>
          ...
          <div id="form-channel-creation-type"
            ...
            <p>Select a type:</p>
            <div className="m-2">
              <input type="radio" id="type" name="publicType" value={type} />
              <label htmlFor="publicType">Public</label>
            </div>
            <div className="m-2">
              <input type="radio" id="type" name="privateType" value={type} />
              <label htmlFor="privateType">Private</label>
            </div>
            <div className="m-2">
              <input type="radio" id="type" name="protectedType" value={type} />
